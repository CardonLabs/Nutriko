...
namespace FdcAgent.Services.FoodStreamService
{
    ...
    public class FdcAgentHttpClient : IFdcAgentHttpClient
    {
        ...
        private JsonSerializerOptions _opt = new JsonSerializerOptions{
        ...
        //         Console.WriteLine($"########## item: {JsonSerializer.Serialize<NuFoodItem>(foodItem)}");
        ...
        public async Task<IList<NuFoodItem>> GetFoods2(IList<int> fdcIds)
        {
            ...
            foreach (var id in fdcIds)
            {
                httpTasks.Add(
                    Task.Run( async() => {
                        ...
                        try
                        {
                            FdcRequestParams2 rp = new FdcRequestParams2(){
                                fdcIds = new List<int> { id },
                                format = _dataSources.Usda.RequestBody.format,
                                nutrients = _dataSources.Usda.RequestBody.nutrients
                            };

                            var jcon = new StringContent(
                                JsonSerializer.Serialize<FdcRequestParams2>(rp, _opt), 
                                Encoding.UTF8, 
                                "application/json"
                            );
                            ...
                            using var responseStream = await response.Content.ReadAsStreamAsync();
                            var item =  await JsonSerializer.DeserializeAsync<IList<SRLegacyFoodItem>>(responseStream, _opt);
            ...
            {
                Console.WriteLine($"--- rsps -- {item.id} - {item.name}");
            }
